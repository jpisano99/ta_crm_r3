...
#
saas_lookup_dict = {}
wb, ws = tool.open_wb('TA SaaS Customer Tracking as of 02-03-20.xlsx')
# saas_tracking_rows = tool.get_list_from_ss(app_cfg['SS_SAAS'])
print()
print('There are', ws.nrows, ' SaaS Tracking rows')
print()
for row_num in range(1, ws.nrows):
    saas_cust_name = ws.cell_value(row_num, 2)
    saas_cust_id = ws.cell_value(row_num, 3)
    saas_telemetry_name = ws.cell_value(row_num, 4)
    saas_vrf = ws.cell_value(row_num, 5)
    saas_so = ws.cell_value(row_num, 6)
    saas_lookup_dict[saas_cust_name] = [saas_cust_id, saas_telemetry_name, saas_vrf, saas_so]
...
for my_id_info in customer_ids:
    ...
    bookings = Bookings.query.filter_by(end_customer_global_ultimate_id=customer_id).all()
    print('Customer ID', customer_id, ' has ', len(my_id_info.customer_aliases),
          ' aliases with', len(bookings), ' bookings')

    for alias_num, my_alias in enumerate(my_id_info.customer_aliases):
        customer_name = my_alias.customer_alias

        # Get Telemetry Info
        if customer_name in saas_lookup_dict:
            telemetry_name = saas_lookup_dict[customer_name][1]
            if telemetry_name == '':
                telemetry_name = 'Non Active on SaaS'
        else:
            telemetry_name = 'Not Found on SaaS'

        # Perform Queries for the Customer Alias
        services = Services.query.filter_by(end_customer=customer_name).all()
        subs = Subscriptions.query.filter_by(end_customer=customer_name).all()
        telemetry = Telemetry.query.filter_by(name=telemetry_name).all()

        print('\tAlias Num '+str(alias_num+1), customer_name, 'has', len(services),
              'Service Engagements and', len(subs), 'Subscriptions', '  SaaS:', telemetry_name)
